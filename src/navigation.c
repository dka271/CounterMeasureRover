...
bool DoWeCrossLineQuestionMark(){
    ...
    return false;
    //Do not cross any lines until orientation is found
    if (movementState < STATE_ORIENTATION_FOUND){
        return false;
    }
    //Decide if this is the right line to cross
...
void GoToRandomLine(bool rotate){
    ...
    if (rotate){
        AddMovement(deg2tick(105), ROVER_DIRECTION_RIGHT);
    }
...
//                                    orientationReceived = true;
                                ...
                                if (orientationReceived){
                                    ...
                                    AddMovement(cm2tick(6), ROVER_DIRECTION_BACKWARDS);
                                    SetMovementGoal();
                                    pathfindingCount = 0;
                                }else{
...
//                                SetOrientation(endX);
                    ...
                    if (addCommand){
                        ...
                        if (moveLastIdx <= moveMaxIdx){
                            AddMovement(distanceTicks, ROVER_DIRECTION_FORWARDS);
                        }
